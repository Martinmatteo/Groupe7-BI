...
perf_produit = csv.writer(open("perf_produit.csv", "wb"))
perf_produit.writerow(["date","type","marge","cout_achat","CA","cout_var","EBE"])
...
#calcule la duree de jours à considerer selon les données du wallet transactions
init=datetime.datetime.strptime(cr[1][0].split(" ")[0],'%Y-%m-%d')
fin=datetime.datetime.strptime(cr[len(cr)-1][0].split(" ")[0], '%Y-%m-%d')
duree=fin-init

for t in range(duree.days+1):
	# **** stock à temps j+1
							...
							if q<0:		
								...
								for rowmarge in margetab:
									if rowmarge[0]==d and rowmarge[1]==row[3]:
										...
										rowmarge[3]=rowmarge[3]+float(rowtab[3])*(float(row[2])-float(q))
						  				bolean=1
						  		if bolean==0:
						  			marge=(float(row[4]) - float(rowtab[3]))*(float(row[2])-float(q))
						  			cout= float(rowtab[3])*(float(row[2])-float(q))
						  			li=[d,row[3],marge,cout]
						  			margetab.append(li)
							else:
								rowtab[2] = q
								for rowmarge in margetab:
									if rowmarge[0]==d and rowmarge[1]==row[3]:
										...
										rowmarge[3]=rowmarge[3]+float(rowtab[3])*float(row[2])
										bolean=1
								if bolean==0:
									marge=(float(row[4]) - float(rowtab[3]))*float(row[2])
									cout=float(rowtab[3])*float(row[2])
									li=[d,row[3],marge,cout]
						  			margetab.append(li)
								break
						else:
							...
							if q<0:		
								...
								for rowmarge in margetab:
									if rowmarge[0]==d and rowmarge[1]==row[3]:
									   ...
									   rowmarge[3]=rowmarge[3]+float(rowtab[3])*float(rowtab[2])
								rowtab[2]=0
							else:
								for rowmarge in margetab:
									if rowmarge[0]==d and rowmarge[1]==row[3]:
									   rowmarge[2] = rowmarge[2] + (float(row[4]) - float(rowtab[3]))*float(int(rowtab[2])-q)
									   rowmarge[3]=rowmarge[3]+float(rowtab[3])*float(int(rowtab[2])-q)
							 	rowtab[2]=q
								break
				if q<0:
...
for row in margetab:
		CA=row[2]+row[3]
		cout_var=0.05*(row[2]+2*row[3])
		EBE=row[2]-cout_var
		row=row+[CA,cout_var,EBE]
		perf_produit.writerow(row)		
print len(margetab) 
